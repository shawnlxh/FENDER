...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.report == 1:
        ...
        for i, users_to_test in enumerate(users_to_test_list):
            ...
            final_perf = "recall=[%s], precision=[%s], hit=[%s], ndcg=[%s], TFR=[%s]" % \
                         ('\t'.join(['%.5f' % r for r in ret['recall']]),
                          '\t'.join(['%.5f' % r for r in ret['precision']]),
                          '\t'.join(['%.5f' % r for r in ret['hit_ratio']]),
                          '\t'.join(['%.5f' % r for r in ret['ndcg']]),
                          '\t'.join(['%.5f' % r for r in ret['TFR']]))
            print(final_perf)
    ...
    *********************************************************
    Train.
    """
    loss_loger, pre_loger, rec_loger, ndcg_loger, hit_loger, TFR_loger = [], [], [], [], [], []
    stopping_step = 0
    should_stop = False
    ...
    for epoch in range(args.epoch):
        ...
        *********************************************************
        ...
        TFR_loger.append(ret['TFR'])

        if args.verbose > 0:
            perf_str = 'Epoch %d [%.1fs + %.1fs]: train==[%.5f=%.5f + %.5f], recall=[%.5f, %.5f], ' \
                       'precision=[%.5f, %.5f], hit=[%.5f, %.5f], ndcg=[%.5f, %.5f], TFR=[%.5f, %.5f]' % \
                       (epoch, t2 - t1, t3 - t2, loss, base_loss, reg_loss, ret['recall'][0], ret['recall'][-1],
                        ret['precision'][0], ret['precision'][-1], ret['hit_ratio'][0], ret['hit_ratio'][-1],
                        ret['ndcg'][0], ret['ndcg'][-1], ret['TFR'][0], ret['TFR'][-1])
            print(perf_str)
    ...
    f.write('embed_size=%d, lr=%.4f, layer_size=%s, node_dropout=%s, mess_dropout=%s, regs=%s, adj_type=%s, use_att=%s, pretrain=%d\n\t%s\n'
            % (args.embed_size, args.lr, args.layer_size, args.node_dropout, args.mess_dropout, args.regs, args.adj_type, args.use_att, args.pretrain, final_perf))
    f.close()
